...
public class CareHistoryFormUploadSchedulesTest extends OpenMRSAwareFunctionalTest {
    ...
    public void shouldNotCreateSchedulesWhileHistoryFormUploadIfThereAreActiveSchedules() {
        ...
        assertAlertDate(scheduleTracker.firstAlertScheduledFor(openMRSId, ANC_IPT_VACCINE.getName()).getAlertAsLocalDate(), expectedFirstAlertDate(ANC_IPT_VACCINE.getName(), pregnancyIn12thWeekOfPregnancy.dateOfConception()));
        assertAlertDate(scheduleTracker.firstAlertScheduledFor(openMRSId, TT_VACCINATION.getName()).getAlertAsLocalDate(), expectedFirstAlertDate(TT_VACCINATION.getName(), registrationDate));
    ...
    public void shouldCreateAppropriateSchedulesWhenCareHistoryUploadedForPatientRegisteredWithANC() {
        ...
        assertAlertDate(scheduleTracker.firstAlertScheduledFor(openMRSId, ANC_IPT_VACCINE.getName()).getAlertAsLocalDate(), scheduleTracker.getActiveMilestone(openMRSId, ANC_IPT_VACCINE.getName()),
    ...
    public void shouldCreateAppropriateSchedulesWhenCareHistoryUploadedForPatientregisteredWithCWC() {
        ...
        String openMRSId = openMRSDB.getOpenMRSId(patientId);
        assertAlertDate(scheduleTracker.firstAlertScheduledFor(openMRSId, CWC_IPT_VACCINE.getName()).getAlertAsLocalDate(), scheduleTracker.getActiveMilestone(openMRSId, CWC_IPT_VACCINE.getName()),
                today().plusWeeks(1), IPTiDose.IPTi2.name());
        assertAlertDate(scheduleTracker.firstAlertScheduledFor(openMRSId, CWC_PENTA.getName()).getAlertAsLocalDate(), scheduleTracker.getActiveMilestone(openMRSId, CWC_PENTA.getName()),
                today().plusWeeks(1), PentaDose.PENTA2.milestoneName());
        assertAlertDate(scheduleTracker.firstAlertScheduledFor(openMRSId, CWC_OPV_OTHERS.getName()).getAlertAsLocalDate(), scheduleTracker.getActiveMilestone(openMRSId, CWC_OPV_OTHERS.getName()),
                today().plusWeeks(1), OPVDose.OPV_2.milestoneName());
    }

    // Separated out since birthdate needs to be different
    @Test
    public void shouldRotavirusSchedulesWhenCareHistoryUploadedForPatientregisteredWithCWC() {
        String staffId = staffGenerator.createStaff(browser, homePage);

        LocalDate registrationDate = today();
        LocalDate birthDate = registrationDate.minusWeeks(9);
        TestPatient patient = TestPatient.with("CWCHistory " + dataGenerator.randomString(5), staffId)
                .patientType(TestPatient.PATIENT_TYPE.CHILD_UNDER_FIVE).estimatedDateOfBirth(false)
                .registrationDate(DateUtil.today()).dateOfBirth(birthDate);
        String patientId = patientGenerator.createPatient(patient, browser, homePage);
        ...
        TestCareHistory careHistory = TestCareHistory.withoutHistory(patientId).staffId(staffId).withRotavirus("1", birthDate.plusWeeks(9));
        ...
        String openMRSId = openMRSDB.getOpenMRSId(patientId);
        assertAlertDate(scheduleTracker.firstAlertScheduledFor(openMRSId, CWC_ROTAVIRUS.getName()).getAlertAsLocalDate(), scheduleTracker.getActiveMilestone(openMRSId, CWC_ROTAVIRUS.getName()),
                today().plusWeeks(3), RotavirusDose.ROTAVIRUS2.milestoneName());
    }
